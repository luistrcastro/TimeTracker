  ...
  try {
    ...
    if (local.length > 0) {
      ...
      if (newOnes.length > 0) {
        ...
        console.log(`Migrated ${newOnes.length} entries from localStorage to file.`);
      }
    }
  } catch {
    _usingServer = false;
  ...
  try {
    ...
    if (!_usingServer) {
      ...
      if (newOnes.length > 0) {
        await persistEntries();
        localStorage.removeItem(STORAGE_KEY);
        console.log(`Heartbeat: recovered ${newOnes.length} entries from localStorage.`);
      }
      setServerStatus(true);
...
function setServerStatus(online) {
  ...
  if (!dot) return;
  dot.title = online ? 'Saving to file (server running)' : 'Saving to browser (server not found)';
...
function applyTheme() {
  ...
  track.style.background = _darkMode ? 'rgba(255,255,255,0.5)' : 'rgba(255,255,255,0.2)';
}
...
function showUndoToast() {
  ...
  const t = document.getElementById('copyToast');
  t.innerHTML = 'Entry deleted &nbsp;<button onclick="undoDelete()" style="background:#fff;color:#1a1917;border:none;border-radius:4px;padding:2px 8px;font-family:var(--mono);font-size:0.75rem;cursor:pointer;font-weight:600;">Undo</button>';
  t.classList.add('show');
...
function renderAll() {
  renderDayView();
  if (document.getElementById('tab-week').style.display !== 'none') renderWeekView();
  if (document.getElementById('tab-replicon').style.display !== 'none') renderRepliconView();
...
function switchTab(name) {
  ...
  document.querySelectorAll('.tab').forEach((btn,i) => {
    btn.classList.toggle('active', ['day','week','replicon','data'][i] === name);
  });
  if (name==='week') renderWeekView();
  if (name==='data') { renderStats(); populateSettingsTab(); }
...
function updateSplitRemaining() {
  ...
  badge.className = 'remaining-badge' + (remainingMins === 0 ? ' balanced' : remainingMins < 0 ? ' over' : '');
  ...
  if (lastFinish && _splitOriginalFinish && lastFinish !== _splitOriginalFinish) {
    const deltaMins = timeToMinutes(lastFinish) - timeToMinutes(_splitOriginalFinish);
    const affected = loadEntries().filter(e =>
      e.date === currentDate &&
      e.id !== _splitSourceId &&
    ...
    const dh = Math.floor(absD / 60), dm = absD % 60;
    const dlabel = dh > 0 ? `${dh}h ${dm}m` : `${dm}m`;
    cascadeBtn.disabled = affected === 0;
    cascadeBtn.textContent = affected > 0
